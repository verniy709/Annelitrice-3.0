...
{
	public class JobDriver_ResurrectUsingegg : JobDriver
	{
		private egg egg => (egg)job.GetTarget(TargetIndex.A).Thing;

		private Thing Item => job.GetTarget(TargetIndex.B).Thing;

		public override bool TryMakePreToilReservations(bool errorOnFailed)
		{
			if (pawn.Reserve(egg, job, 1, -1, null, errorOnFailed))
			{
		...
		private void Resurrect()
		{
			...
			GenSpawn.Spawn(innerPawn, egg.PositionHeld, egg.MapHeld);
			egg.Destroy();
