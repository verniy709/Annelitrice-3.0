...
namespace Annelitrice
{
	...
	public class JobDriver_InfestHuman : JobDriver
    {
		...
		protected override IEnumerable<Toil> MakeNewToils()
		{
			...
			toil.tickAction = delegate
			{
				...
				if (curDuration >= WorkTimeTicks)
				{
					...
					var hediff = HediffMaker.MakeHediff(AnnelitriceDefOf.Anneli_Infection, Target);
		...
		{
			Pawn friend = Target;
			if (friend.CurJobDef != AnnelitriceDefOf.Anneli_StandStill)
			{
				Job job = JobMaker.MakeJob(AnnelitriceDefOf.Anneli_StandStill, pawn);
